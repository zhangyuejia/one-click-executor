...
public class CmdPullCodeServiceImpl extends AbstractCmdService<CmdPullCodeConfig> {
    ...
    @Override
    public void exec(CmdPullCodeConfig config) throws Exception {
        initConfig(config);
        for (ModulePropertiesBO moduleProperties : config.getModulesProperties()) {
            ...
            pullModuleCodeAsync(config, moduleProperties);
        }
    }

    private void pullModuleCodeAsync(CmdPullCodeConfig config, ModulePropertiesBO moduleProperties) throws InterruptedException {
        AsyncTaskGroup asyncTaskGroup = TaskGroupFactory.newAsyncTaskGroup();
        for (ModulePropertiesBO.ModulesParam modulesParam : moduleProperties.getModulesParams()) {
            asyncTaskGroup.execAsync(() -> {
                try {
                    pullModuleCode(config, moduleProperties, modulesParam);
                } catch (Exception e) {
    ...
    private void pullModuleCode(CmdPullCodeConfig config, ModulePropertiesBO moduleProperties, ModulePropertiesBO.ModulesParam modulesParam) throws Exception {
    ...
    private void initConfig(CmdPullCodeConfig config) {
